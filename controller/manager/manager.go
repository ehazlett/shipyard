...
type (
	...
	Manager interface {
		...
		AddImagesToProjectId(project *model.Project) error
...
func (m DefaultManager) AddImagesToProjectId(project *model.Project) error {
	var eventType string
	// check if exists; if so, update
	proj, err := m.Project(project.ID)
	if err != nil && err != ErrProjectDoesNotExist {
	...
	if proj != nil {
		...
		//Insert all the images again in the tblNameImages
		for _, newImage := range project.Images {
			newImage.ProjectID = proj.ID
			if _, err := r.Table(tblNameImages).Insert(newImage).RunWrite(m.session); err != nil {
				return err
