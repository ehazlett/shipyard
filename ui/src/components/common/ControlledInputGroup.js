        ...
        {value.map((obj, idx) => {
          return (
            <Form.Group key={value.indexOf(obj)} widths="equal">
              {columns.map((column) => {
                ...
                return (
                  <Form.Field key={columns.indexOf(column)}>
                    ...
                    {
                      column.component ?
                        <column.component name={columnName} value={columnValue} onChange={this.onChangeHandler} type={column.type} {...column.props} /> :
                        <Input name={columnName} value={columnValue} onChange={this.onChangeHandler} type={column.type} {...column.props}/>
                    }
                  </Form.Field>
                );
              })}
              <Form.Field width={1}>
                <label>&nbsp;</label>
                <Button type="button" icon="minus" color="red" onClick={() => { this.remove(obj); }} />
              </Form.Field>
            </Form.Group>
          );
        })}
