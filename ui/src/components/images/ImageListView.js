  ...
  render() {
    ...
    const columns = [
        ...
        render: row => {
          ...
          return (
            <Checkbox
              checked={selected}
              onChange={() => {
                this.selectItem(row.rowValues.Id);
              }}
              className={selected ? "active" : ""}
              key={row.rowValues.Id}
            />
          );
        },
        sortable: false,
      ...
      },
      {
        ...
        id: "Repository",
        accessor: d =>
          d.RepoTags === undefined || d.RepoTags === null ? "" : d.RepoTags[0],
        render: row => {
      ...
      },
      {
        ...
        render: row => {
          return (
            <Link to={`/images/inspect/${row.rowValues.Id}`}>
              {row.value.split(":")[1]}
            </Link>
          );
        },
      ...
      },
      {
        ...
        render: row => {
          return (
            <Link to={`/images/inspect/${row.rowValues.Id}`}>
              {row.value.replace("sha256:", "").substring(0, 12)}
            </Link>
          );
        },
        sortable: true
